 ...
 * @return {Bool}             True if STARTTLS is required.
 */
const isStartTlsRequired = smtpReply => {
  if (
    smtpReply &&
    /^(530)/.test(smtpReply) &&
    /(starttls)/ig.test(smtpReply)
  ) return true;
...
class EmailValidator {
  ...
  static async verifyMailbox(local, domain, [mxRecord], timeout, log, fromAddress, useOpenSSL, portList) {
    if (/yahoo/.test(mxRecord)) {
      log.debug('Yahoo mailbox not supported', mxRecord);
      return null;
    }
    if (!mxRecord) {
    ...
    const handleSession = (resolve, portIdx, _useOpenSSL = false) => {
      const port = portList[portIdx]
      const socket = _useOpenSSL
      ...
      socket.on('data', data => {
        ...
        if (isStartTlsRequired(data)) return trySSL();
      ...
      const trySSL = () => {
        if (useOpenSSL) {
          handleSession(resolve, portIdx, true);
        } else {
          ret(null);
        }
      }
